...
CPacketDecoder RoomHandlerProxy::_decoder;

int RoomHandlerProxy::OnAttachPoller(CTCPSocketHandler * phandler)
{
...
int RoomHandlerProxy::OnConnected(CTCPSocketHandler * phandler)
{
	//句柄被Accept后触发
	//todo 为了防止内存泄漏 应该先检测一下 handler是否已经绑定过
	RoomHandlerToken * ptoken = new RoomHandlerToken(phandler);
	HandlerManager::Instance()->BindHandlerToken(phandler,ptoken);
	return 0;
}

int RoomHandlerProxy::OnConnectedByRequest(CTCPSocketHandler * phandler)
{
...
int RoomHandlerProxy::OnClose(CTCPSocketHandler * phandler)
{
	//HandlerManager::Instance()->UnBindHandlerToken(phandler);
	return 0;
}

int RoomHandlerProxy::ProcessPacket(RoomHandlerToken * ptoken,const PBHead & head,const PBCSMsg & msg,const PBRoute & route)
{
...
int RoomHandlerProxy::OnPacketComplete(CTCPSocketHandler * phandler,const char * data,int len)
{
	RoomHandlerToken * ptoken =(RoomHandlerToken *) HandlerManager::Instance()->GetHandlerToken(phandler);
	if(ptoken==NULL)
	{
...
int RoomHandlerProxy::OnProcessOnTimerOut(CTCPSocketHandler * phandler,int Timerid)
{
	return 0;
}

void RoomHandlerProxy::OnProcessInnerMessage(const PBCSMsg & innermsg)
{
