...
GlobalRedisClient * GlobalRedisClient::Instance()
{
    return CSingleton<GlobalRedisClient>::Instance();
}

bool GlobalRedisClient::Init()
{
    if ( !PokerPBGlobalRedisConfig::Instance()->Init("../../conf/global_redis.CFG") )
    {
        VLogMsg(CLIB_LOG_LEV_ERROR,"failed to load global redis config");
        return false;
    }
    if (Connect(PokerPBGlobalRedisConfig::Instance()->ip(),PokerPBGlobalRedisConfig::Instance()->port()) != 0 )
    {
        VLogMsg(CLIB_LOG_LEV_ERROR,"failed to connect to global redis[ip:%s,port:%d]",
                PokerPBGlobalRedisConfig::Instance()->ip().c_str(),PokerPBGlobalRedisConfig::Instance()->port());
        return false;
    }
    return true;
}

bool GlobalRedisClient::GetHashMapValue(const string & mainkey,const string & subkey,string & value)
{
...
bool GlobalRedisClient::GetHashMapValue(const string & mainkey,const string & subkey,int64 & value)
{
    m_pRedisReply = RedisVCommand("hget %s %s",mainkey.c_str(),subkey.c_str());
    if (!m_pRedisReply)
    {
...
bool GlobalRedisClient::SetHashMapValue(const string & mainkey,const string & subkey,int64 value)
{
...
bool GlobalRedisClient::SetHashMapValue(const string & mainkey,const string & subkey,const string & value)
{
...
bool GlobalRedisClient::DelHashMapKey(const string & mainkey,const string & subkey)
{
...
bool GlobalRedisClient::GetAllHashMapKey(const string & mainkey,vector<string> & strvect)
{
    strvect.clear();
...
bool GlobalRedisClient::GetAllHashMapKey(const string & mainkey,vector<int64> & tidvect)
{
    tidvect.clear();
    m_pRedisReply = RedisVCommand("HKEYS %s",mainkey.c_str());
...
bool GlobalRedisClient::QueryHashObject(const string & mainkey,const string & subkey,PBHashDataField & pb_hash_data_field)
{
    char buff[102400] = {0};
    int len;
...
bool GlobalRedisClient::QueryHashObjectForReply(const string & mainkey,const string & subkey,PBHashDataField & pb_hash_data_field)
{
    char buff[102400] = {0};
    int len;
    int iret = _QueryHashObject(mainkey,subkey,buff,len);
...
int GlobalRedisClient::_QueryHashObject(const string & mainkey,const string & subkey,char * buff,int & len)
{
    m_pRedisReply = RedisVCommand("hget %s %s",mainkey.c_str(),subkey.c_str());
...
bool GlobalRedisClient::UpdateHashObject(const string & mainkey,const string & subkey,const PBHashDataField & pb_hash_data_field)
{
...
int GlobalRedisClient::_UpdateHashObject(const string & mainkey,const string & subkey,PBRedisData & data)
{
...
void GlobalRedisClient::IncreaseHashObject(const string & mainkey,const string & subkey, int64_t inc)
{
...
int64_t GlobalRedisClient::GetHashObject(const string & mainkey,const string & subkey)
{
    int64_t val = 0;
...
bool GlobalRedisClient::CheckExist(const string & mainkey,const string & subkey)
{
