...
int ClientHandlerProxy::DoTransfer(ConnectHandlerToken * ptoken,CSession * psession,CWSPBInputPacket & ws_input)
{
	int cmd = psession->_head.cmd();
	CPBInputPacket input;
	if (!input.CoverFrom(ws_input))
	{
		VLogMsg(CLIB_LOG_LEV_DEBUG, "conver from ws input failed");
