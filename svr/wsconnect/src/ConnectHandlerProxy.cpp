...
int ConnectHandlerProxy::DoTransfer(CSession * psession,CPBInputPacket & input)
{
	...
	else
	{
		...
		if (PokerPBConnectSvrdConfig::Instance()->IsGameNode((ENNodeType)psession->_request_route.source()))
		{
			Message::PushInnerMsg(RouteManager::Instance()->GetRouteByRandom(), uid, msg, (ENNodeType)psession->_request_route.source(), psession->_request_route.source_id());
