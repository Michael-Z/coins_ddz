...
//        //闯关模式中加入了闯关的信息
//        const PBUserSkipMatchInfo & user_skip_match_info = ss_response.data_set().user_data().user_skip_match_info();
//        response.mutable_user_skip_match_info()->CopyFrom(user_skip_match_info);
		...
		Message::SendMsgToH5Client(pUserToken->_phandler, psession, msg);
		// 更新用户IP
        // 通知用户状态
        PBUpdateData update;
...
ENHandlerResult CRequestEnterTable::ProcessRequestMsg(CHandlerTokenBasic * ptoken,CSession * psession)
{
	...
	if(pUserToken)
	{
		const CSRequestEnterTable & cs_request_enter_table = psession->_request_msg.cs_request_enter_table();
		Message::SendRequestMsg(RouteManager::Instance()->GetRouteByRandom(),psession,psession->_request_msg,
								EN_Node_Room,1);
        VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] request enter table[%ld] of server[%d].",
...
ENHandlerResult CRequestEnterTable::ProcessResponseMsg(CHandlerTokenBasic* ptoken, CSession* psession)
{
	...
	if (pUserToken != NULL)
	{
		...
		const CSResponseEnterTable & cs_response_enter_table = psession->_response_msg.cs_response_enter_table();
		if(cs_response_enter_table.result() == EN_MESSAGE_ERROR_OK)
		{
			pUserToken->_pos.set_pos_type(EN_Position_Table);
            pUserToken->_pos.set_gamesvrd_id(cs_response_enter_table.gamesvrd_id());
            VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] enter table of game[%d] succ,session[%d]",
...
ENHandlerResult CRequestFPFEnterTable::ProcessRequestMsg(CHandlerTokenBasic * ptoken,CSession * psession)
{
    // todo
    //请求roomsvrd 获取当前牌桌的gamesvrd id
    int64 uid = psession->_uid;
    ...
    if(pUserToken)
    {
        const CSRequestFPFEnterTable & cs_request_enter_table = psession->_request_msg.cs_request_fpf_enter_table();
        Message::SendRequestMsg(RouteManager::Instance()->GetRouteByRandom(),psession,psession->_request_msg,
                                EN_Node_Room,1);
        VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] request enter fpf table[%d] of server[%d].",
            uid,cs_request_enter_table.tid(),1);
        return EN_Handler_Succ;
    }
    else
...
ENHandlerResult CRequestFPFEnterTable::ProcessResponseMsg(CHandlerTokenBasic* ptoken, CSession* psession)
{
    long long uid = psession->_uid;
    ConnectHandlerToken * pUserToken = AccessUserManager::Instance()->GetHandlerTokenByUid(uid);
    if (pUserToken != NULL)
    {
        ...
        output.End();
        pUserToken->_phandler->Send(output.packet_buf(),output.packet_size());
        const CSResponseFPFEnterTable & cs_response_enter_table = psession->_response_msg.cs_response_fpf_enter_table();
        if(cs_response_enter_table.result() == EN_MESSAGE_ERROR_OK)
        {
            pUserToken->_pos.set_pos_type(EN_Position_Fpf);
            pUserToken->_pos.set_gamesvrd_id(cs_response_enter_table.gamesvrd_id());
            VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] enter table of fpf fpf game[%d] succ,session[%d]",
                uid,cs_response_enter_table.gamesvrd_id(),psession->sessionid());
        }
    }
    return EN_Handler_Done;
...
ENHandlerResult CRequestDaerEnterTable::ProcessRequestMsg(CHandlerTokenBasic * ptoken,CSession * psession)
{
    // todo
    //请求roomsvrd 获取当前牌桌的gamesvrd id
    int64 uid = psession->_uid;
    ...
    if(pUserToken)
    {
        const CSRequestDaerEnterTable & cs_request_enter_table = psession->_request_msg.cs_request_daer_enter_table();
        Message::SendRequestMsg(RouteManager::Instance()->GetRouteByRandom(),psession,psession->_request_msg,
                                EN_Node_Room,1);
        VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] request enter daer table[%d] of server[%d].",
            uid,cs_request_enter_table.tid(),1);
        return EN_Handler_Succ;
    }
    else
...
ENHandlerResult CRequestDaerEnterTable::ProcessResponseMsg(CHandlerTokenBasic* ptoken, CSession* psession)
{
    long long uid = psession->_uid;
    ConnectHandlerToken * pUserToken = AccessUserManager::Instance()->GetHandlerTokenByUid(uid);
    if (pUserToken != NULL)
    {
        ...
        output.End();
        pUserToken->_phandler->Send(output.packet_buf(),output.packet_size());
        const CSResponseDaerEnterTable & cs_response_enter_table = psession->_response_msg.cs_response_daer_enter_table();
        if(cs_response_enter_table.result() == EN_MESSAGE_ERROR_OK)
        {
            pUserToken->_pos.set_pos_type(cs_response_enter_table.pos_type());
            pUserToken->_pos.set_gamesvrd_id(cs_response_enter_table.gamesvrd_id());
            VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] enter table of daer game[%d] succ,session[%d]",
                uid,cs_response_enter_table.gamesvrd_id(),psession->sessionid());
        }
    }
    return EN_Handler_Done;
...
ENHandlerResult CRequestDssEnterTable::ProcessRequestMsg(CHandlerTokenBasic * ptoken,CSession * psession)
{
    // todo
    //请求roomsvrd 获取当前牌桌的gamesvrd id
    int64 uid = psession->_uid;
...
ENHandlerResult CRequestDssEnterTable::ProcessResponseMsg(CHandlerTokenBasic* ptoken, CSession* psession)
{
    long long uid = psession->_uid;
    ConnectHandlerToken * pUserToken = AccessUserManager::Instance()->GetHandlerTokenByUid(uid);
    if (pUserToken != NULL)
    {
        ...
        output.End();
        pUserToken->_phandler->Send(output.packet_buf(),output.packet_size());
        const CSResponseDssEnterTable & cs_response_enter_table = psession->_response_msg.cs_response_dss_enter_table();
        if(cs_response_enter_table.result() == EN_MESSAGE_ERROR_OK)
        {
            pUserToken->_pos.set_pos_type(cs_response_enter_table.pos_type());
            pUserToken->_pos.set_gamesvrd_id(cs_response_enter_table.gamesvrd_id());
...
ENHandlerResult CResponseDssEnterTable::ProcessRequestMsg(CHandlerTokenBasic* ptoken, CSession* psession)
{
	long long uid = psession->_uid;
	ConnectHandlerToken * pUserToken = AccessUserManager::Instance()->GetHandlerTokenByUid(uid);
	if (pUserToken != NULL)
	{
		...
		output.End();
		pUserToken->_phandler->Send(output.packet_buf(), output.packet_size());
		const CSResponseDssEnterTable & cs_response_enter_table = psession->_request_msg.cs_response_dss_enter_table();
		if (cs_response_enter_table.result() == EN_MESSAGE_ERROR_OK)
		{
			pUserToken->_pos.set_pos_type(cs_response_enter_table.pos_type());
			pUserToken->_pos.set_gamesvrd_id(cs_response_enter_table.gamesvrd_id());
...
ENHandlerResult CRequestSdrEnterTable::ProcessRequestMsg(CHandlerTokenBasic * ptoken,CSession * psession)
{
    ...
    if(pUserToken)
    {
        ...
        VLogMsg(CLIB_LOG_LEV_DEBUG,"user[%lld] request enter sdr table[%d] of server[%d].",
            uid,cs_request_enter_table.tid(),1);
        //return EN_Handler_Succ;   //session结束
        return EN_Handler_Done;
    }
    else
    {
