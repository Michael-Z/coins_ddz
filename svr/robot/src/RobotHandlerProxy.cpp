...
CPacketDecoder RobotHandlerProxy::_decoder;

RobotHandlerProxy::RobotHandlerProxy()
{
...
int RobotHandlerProxy::OnAttachPoller(CTCPSocketHandler * phandler)
{
...
int RobotHandlerProxy::OnConnected(CTCPSocketHandler * phandler)
{
	//句柄被Accept后触发
	//todo 为了防止内存泄漏 应该先检测一下 handler是否已经绑定过
	RobotHandlerToken * ptoken = new RobotHandlerToken(phandler);
	HandlerManager::Instance()->BindHandlerToken(phandler,ptoken);
	_reconnect_timer.StopTimer();
	return 0;
}

int RobotHandlerProxy::OnReConnected(CTCPSocketHandler * phandler)
{
	return 0;
}


int RobotHandlerProxy::OnConnectedByRequest(CTCPSocketHandler * phandler)
{
...
int RobotHandlerProxy::OnClose(CTCPSocketHandler * phandler)
{
...
int RobotHandlerProxy::ProcessPacket(RobotHandlerToken * ptoken, const PBHead & head, const PBCSMsg & msg, const PBRoute & route)
{
...
int RobotHandlerProxy::OnPacketComplete(CTCPSocketHandler * phandler, const char * data, int len)
{
...
int RobotHandlerProxy::OnProcessOnTimerOut(CTCPSocketHandler * phandler, int Timerid)
{
	return 0;
}

void RobotHandlerProxy::OnProcessInnerMessage(const PBCSMsg & innermsg)
{
...
int RobotHandlerProxy::ProcessOnTimerOut(int Timerid)
{
	switch (Timerid)
	{
...
void RobotHandlerProxy::ReconnectRoute()
{
	RouteManager::Instance()->Init(this);
}
